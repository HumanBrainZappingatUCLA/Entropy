function x = BENvsAge(basedir,groupname,Transform_matBEN)
    load('C:\Users\tjjordan\Documents\UCLAResearch\TS_Analysis\Behavior\SubJ_Age.mat')
    x=[];
    ...
    saveplots=fullfile(savedir,'Plots','Age');
    ...
    Subage=cell2mat(SubAge);
    k=1;
    for n = 1:2:length(subfiles)
        n2 = convertStringsToChars(subfiles(n));
        n3 = str2num(n2(5:9));
        if ismember(n3,Subage(:,1))
            ind=find(Subage==n3);
            Smoketime(k,:)= Subage(ind(1),:);
            Smokesubs(k)=n3;
            k=k+1;
        end
    end
...
%     SigcompStor05rsq=SigcompStor05(SigcompStor05(:,5)>0.1,1:6);
    ...
    filename=fullfile(savedir,['BENvsAge_PrevsPost_' groupname '.mat']);
    ...
    %     b=p1(2);
        ...
        g(1,1).set_names('x','Age (yrs)','y','Regional SampEn');
        g(1,1).geom_point();
        ...
        g(1,2).set_names('x','Age (yrs)','y','Regional SampEn');
        g(1,2).geom_point();
    ...
    %     plot(x,yA,'LineWidth',2)
        ...
        saveas(gcf, filename);
        close all
        Plotdata2.(['ROI' convertStringsToChars(ROI) '_vsSubJ_Age']).x=Smoker_plot;
        Plotdata2.(['ROI' convertStringsToChars(ROI) '_vsSubJ_Age']).y=BENBp;
        clear g
        x=1;
    end
    filename=fullfile(savedir,'BENplotdataAge.mat');
